...
std::vector<float> EmbeddingInfer::processInput(){
    ...
    torch::Tensor t = torch::tensor(input_tensor_values, {torch::kFloat32});
    //std::cout << t << std::endl;
...
bool EmbeddingInfer::verifyOutput(std::vector<float> output_tensor_values){
    ...
    //std::cout << output <<"\n";
    torch::Tensor prod = torch::tensor(output_tensor_values, {torch::kFloat64});
    std::cout << prod.slice(0,0,100) << std::endl;
    std::cout << dim0 <<" "<< 8 <<"\n";
    
    //read input data
    auto file_path = "datanmodels/out_e1000.csv";
    std::ifstream f (file_path);   /* open file */
    if (!f.is_open()) {     /* validate file open for reading */
        perror (("error while opening file " + std::string(file_path)).c_str());
    }
    ...
    f.close();
    torch::Tensor exp = torch::tensor(input_tensor_values, {torch::kFloat64});
    std::cout << exp.slice(0,0,100) << std::endl;
    //auto input = xt::load_csv<double>(in_file);
    //std::cout << input <<"\n";
    //auto shapeo = input.shape();   // shape = {2, 3}
    //std::cout << shapeo[0] << " " << shapeo[1] << "\n";
    bool same = prod.equal(exp);
    std::cout<<"Are the same: "<< same<<std::endl;
    return true;
}
